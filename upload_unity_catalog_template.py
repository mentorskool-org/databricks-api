    ...
    if selected_option == "Volume":
        ...
        if st.button("Upload Data"):
            with st.spinner(text="In progress...", cache=False):
                if new_volume:
                    try:
                        response = fc.create_volume(catalog_name, schema_name, volume_name)
            ...
            try:
                with st.spinner("Processing..."):
                    response = upload_file_to_table(
                        catalog_name, schema_name, table_name, file_to_upload_location
                    )
                    ...
                    try:
                        ...
                        latest_version = table_versions[0] + 1
                    except Exception as error:
